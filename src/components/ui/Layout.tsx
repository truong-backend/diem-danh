...
const navItems = [
  { to: "/dashboard",  icon: "dashboard",        label: "Dashboard",        roles: ["ADMIN","TEACHER","STUDENT"] },
  { to: "/timetable",  icon: "calendar_today",   label: "Thời khóa biểu",   roles: ["ADMIN","TEACHER","STUDENT"] },
  { to: "/classes",    icon: "book",              label: "Lớp học",          roles: ["ADMIN","TEACHER","STUDENT"] },
  { to: "/courses",    icon: "menu_book",         label: "Môn học",          roles: ["ADMIN"] },
  { to: "/users",      icon: "group",             label: "Người dùng",       roles: ["ADMIN","TEACHER"] },
  { to: "/qr-checkin", icon: "qr_code_scanner",  label: "Điểm danh QR",     roles: ["STUDENT"] },
  { to: "/chat",       icon: "chat",              label: "Tin nhắn",         roles: ["ADMIN","TEACHER","STUDENT"] },
  { to: "/profile",    icon: "account_circle",    label: "Trang cá nhân",    roles: ["ADMIN","TEACHER","STUDENT"] },
];
...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen bg-[#f8f9fb] text-[#191c1e]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      `}>
        ...
        <div className="px-6 pb-8">
          <h1 style={{ fontSize: 20, fontWeight: 900, lineHeight: "28px", color: "#003d9b" }}>
            AttendSys
          </h1>
          <p style={{ fontSize: 12, fontWeight: 500, color: "#434654", letterSpacing: "0.01em" }}>
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <span style={{ fontSize: 12, lineHeight: "16px", letterSpacing: "0.01em", fontWeight: isActive ? 700 : 500 }}>
                    {item.label}
                  </span>
                </>
              )}
      ...
      <main className="flex-1 overflow-auto flex flex-col min-w-0">
        ...
        >
          ...
          <div className="flex items-center flex-1 max-w-xl gap-3">
            <button onClick={() => setMobileOpen(true)} className="p-2 text-[#434654] lg:hidden">
              <Menu size={20} />
            </button>
            <div className="relative w-full hidden sm:block">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#434654]" style={{ fontSize: 20 }}>
                search
              </span>
              <input
