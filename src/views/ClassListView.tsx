...
export default function ClassListView() {
  ...
  const ClassForm = ({
    form, setForm, onSubmit, onClose, saving: isSaving, title,
  }: {
    form: CreateClassRoomPayload;
    setForm: (f: CreateClassRoomPayload) => void;
    ...
    courses.length === 0 ? (
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <input className="input" required placeholder="VD: Lập trình Java - Nhóm 01"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input className="input" required value={form.semester} onChange={e => setForm({ ...form, semester: e.target.value })} />
          </div>
          <div>
            <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant block mb-2">Năm học</label>
            <input className="input" required value={form.academicYear} onChange={e => setForm({ ...form, academicYear: e.target.value })} />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
  ...
  return (
    <div className="p-6 space-y-5">
            ...
            {classes.map((cr) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 text-sm text-on-surface-variant ml-4 shrink-0">
                  ...
                  {cr.schedule && <span className="flex items-center gap-1 hidden sm:flex"><Calendar className="w-4 h-4" />{cr.schedule}</span>}
                </div>
                {isAdmin && (
      ...
      {/* Modal tạo lớp */}
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Tạo lớp học mới" size="md">
        <ClassForm
          form={createForm}
      ...
      {/* Modal sửa lớp */}
      <Modal open={showEdit} onClose={() => { setShowEdit(false); setEditTarget(null) }} title={`Sửa: ${editTarget?.name}`} size="md">
        <ClassForm
          form={editForm}
          setForm={setEditForm}
          onSubmit={handleEdit}
