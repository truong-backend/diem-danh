...
import { Plus, Users, Calendar, ChevronRight, Info, Trash2 } from "lucide-react";
...
export default function ClassListView() {
  const { classes, loading, createClass, deleteClass } = useClassRoomViewModel();
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (ok) {
      setShowModal(false);
      setForm({ name: "", courseId: "", teacherId: "", semester: "HK1", academicYear: "2024-2025", maxStudents: 40, schedule: "" });
    }
  };

  const handleDelete = async (e: React.MouseEvent, classId: string, name: string) => {
    e.stopPropagation(); // Không navigate vào lớp khi click xóa
    if (!confirm(`Xoá lớp học "${name}"?\nToàn bộ buổi học và dữ liệu điểm danh sẽ bị xoá. Hành động này không thể hoàn tác.`)) return;
  ...
  return (
    <div className="p-6 space-y-5">
          ...
          <div className="divide-y divide-outline-variant/10">
            {classes.map((cr) => (
              ...
              >
                ...
                {isAdmin && (
                  <button
                    className="ml-3 p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded opacity-0 group-hover:opacity-100 transition-opacity"
                    title="Xoá lớp học"
                ...
                <ChevronRight className="w-5 h-5 text-on-surface-variant/40 ml-2" />
              </div>
            ))}
            {classes.length === 0 && (
      ...
      {/* Modal tạo lớp */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Tạo lớp học mới" size="md">
        {courses.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <p className="text-sm text-on-surface-variant mt-1">
              Vui lòng vào mục <strong>Môn học</strong> để tạo môn học trước khi tạo lớp.
            </p>
            <button className="btn-secondary mt-4" onClick={() => setShowModal(false)}>Đóng</button>
          </div>
        ) : (
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              ...
              <input
                className="input" required placeholder="VD: Lập trình Java - Nhóm 01"
                value={form.name}
            ...
            <div>
              ...
              <select className="input" required value={form.courseId} onChange={(e) => setForm((f) => ({ ...f, courseId: e.target.value }))}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input className="input" required value={form.semester} onChange={(e) => setForm((f) => ({ ...f, semester: e.target.value }))} />
              </div>
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant block mb-2">Năm học</label>
                <input className="input" required value={form.academicYear} onChange={(e) => setForm((f) => ({ ...f, academicYear: e.target.value }))} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant block mb-2">Sĩ số tối đa</label>
                <input type="number" className="input" min={1} max={200} value={form.maxStudents} onChange={(e) => setForm((f) => ({ ...f, maxStudents: +e.target.value }))} />
              </div>
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-on-surface-variant block mb-2">Lịch học</label>
                <select className="input" required value={form.schedule} onChange={(e) => setForm((f) => ({ ...f, schedule: e.target.value }))}>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" className="btn-secondary" onClick={() => setShowModal(false)}>Huỷ</button>
              <button type="submit" className="btn-primary" disabled={saving}>{saving ? "Đang tạo..." : "Tạo lớp"}</button>
            </div>
          </form>
        )}
      </Modal>
